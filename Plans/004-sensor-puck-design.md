...
| **Display** | 2.13" e-paper, portrait, side-mounted |
...
```
    ...
    └────────────────────────┬────────────────────────┘
                      ...
                      └──────┬──────┘
                             │
    ┌────────────────────────▼───────┬────────┐
    │           BASE SHELL           │ E-Paper│  ← Portrait display
    │  ┌───────┐  ┌───────────────┐  │ + ESP32│     in side bump
    │  │NAU7802│  │   Battery     │  │ Feather│
    │  │LIS3DH │  │   1200mAh     │  ├────────┤
    │  │DS3231 │  │               │  │        │
    │  └───────┘  └───────────────┘  │        │
    └────────────────────────────────┴────────┘
           85mm main body              Side bump
```
...
```
    ...
    │   [Battery]       ├────────┐
    │                   │        │
    └───────────────────┤ E-Paper│ ← Portrait (23mm x 48mm)
              │         │        │
              │         │ ESP32  │ ← Feather behind display
           85mm         └────────┘
                         Side bump (~25mm protrusion)
```
...
### Portrait E-Paper (2.13" FeatherWing)

The Feather + FeatherWing stack mounts vertically in a side bump, giving portrait orientation.

**Display specs:**
- Resolution: 122 x 250 pixels (portrait)
- Physical size: 23mm x 48mm visible area
...
│ Total  │
│        │
│ 1,850  │  ← Cumulative consumption
│   ml   │    (persists across refills)
│        │
│────────│
│        │
...
| Side bump width | 25mm | Houses Feather stack |
| Side bump height | 55mm | Display + Feather |
| Side bump depth | 30mm | Same as main body |

### Side Bump Detail

```
Front view of bump:          Side view of bump:
...
Side bump interior:
    ...
    │  │   Display    │  │    (23 x 48mm cutout)
    │  │              │  │
    │  └──────────────┘  │
    │  ┌──────────────┐  │
    │  │ ESP32 Feather│  │  ← Behind display
    │  │  (51 x 23)   │  │    Stacking headers connect
    │  └──────────────┘  │
    │        ◯ USB-C     │  ← Charging port
    └────────────────────┘
```
...
```
4.1 Mount ESP32 Feather in side bump
4.2 Stack FeatherWing on top (portrait orientation)
4.3 Route STEMMA QT cable from ESP32 to NAU7802
4.4 Connect battery JST to ESP32
```
