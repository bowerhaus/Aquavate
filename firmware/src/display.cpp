...
// Human figure SMILEY FILLED bitmap (50x83 pixels) - overlay when goal reached
const unsigned char PROGMEM human_figure_smiley_filled_bitmap[] = {
    0x00, 0x00, 0x03, 0xB0, 0x00, 0x00, 0x00,
    ...
    0x00, 0x00, 0xF1, 0xF7, 0xC0, 0x00, 0x00,
    0x00, 0x01, 0xF1, 0xE3, 0xE0, 0x00, 0x00,
    0x00, 0x01, 0xF9, 0xF3, 0xE0, 0x00, 0x00,
    0x00, 0x03, 0xFF, 0xFF, 0xF0, 0x00, 0x00,
    0x00, 0x01, 0xFF, 0xFF, 0xF0, 0x00, 0x00,
    0x00, 0x03, 0xFF, 0xFF, 0xE0, 0x00, 0x00,
    0x00, 0x03, 0xEF, 0xFB, 0xF0, 0x00, 0x00,
    0x00, 0x01, 0xE7, 0xF1, 0xF0, 0x00, 0x00,
    0x00, 0x03, 0xF1, 0xE3, 0xF0, 0x00, 0x00,
...
#define HUMAN_FIGURE_SMILEY_WIDTH 50
#define HUMAN_FIGURE_SMILEY_HEIGHT 83
...
static void drawHumanFigure(int x, int y, float fill_percent, bool goal_reached) {
    ...
    for (int row = 0; row < HUMAN_FIGURE_HEIGHT; row++) {
        ...
        // Select bitmap: use smiley version if goal reached and figure is filled, otherwise use normal progression
        const unsigned char* bitmap_to_use;
        if (goal_reached && row >= fill_start_row) {
            // Use smiley filled bitmap when goal is reached and this row should be filled
            bitmap_to_use = human_figure_smiley_filled_bitmap;
        } else if (row >= fill_start_row) {
            // Use regular filled bitmap when not at goal
            bitmap_to_use = human_figure_filled_bitmap;
        } else {
            // Use outline bitmap for unfilled portion
